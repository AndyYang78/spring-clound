...
$(function () {
    ...
    $('#input_edit_color').colorpicker();

    $.get('/content/chatroom/template/list', function (response) {
        if (response.success) {
            for (var i in response.data) {
                $('#input_tamplate').append('<option value="' + response.data[i].id + '">' + response.data[i].name + '</option>');
                $('#input_edit_tamplate').append('<option value="' + response.data[i].id + '">' + response.data[i].name + '</option>');
            }
        } else {
...
window.operateEvents = {
    ...
    'click .edit': function (e, value, row) {
        ...
        if (row.chatRoomTemplate != null)
            $('#input_edit_tamplate').val(row.chatRoomTemplate.id).trigger("change");
        else
            $('#input_edit_tamplate').val('').trigger("change");
        $('#input_edit_color').colorpicker('setValue', row.color);
    ...
    'click .remove': function (e, value, row) {
        ...
        $.post('/content/chatroom/type/remove', {id: value}, function (response) {
            if (response.success) {
                $('#table_main').bootstrapTable('refresh');
    ...
    'click .open': function (e, value) {
        //开始提交
        $.post('/content/chatroom/type/open', {id: value}, function (response) {
            if (response.success) {
                $('#table_main').bootstrapTable('refresh');
    ...
    'click .shutdown': function (e, value) {
        //开始提交
        $.post('/content/chatroom/type/close', {id: value}, function (response) {
            if (response.success) {
                $('#table_main').bootstrapTable('refresh');
...
function commitType() {
    ...
    var param = {};
    param['templateId'] = $('#input_tamplate').val();
    param['name'] = name;
    param['color'] = color;
    //开始提交
    $.post('/content/chatroom/type/create', param, function (response) {
        log(response);
        if (response.success) {
            $('#table_main').bootstrapTable('refresh');
...
function commitUpdate() {
    ...
    var color = $('#input_edit_color input').val();
    if (color === '') {
        return note('颜色值不能为空！', 'warning', 1500, 'topRight');
    }
    var param = {};
    ...
    param['name'] = name;
    param['color'] = color;
    param['templateId'] = $('#input_edit_tamplate').val();
    //开始提交
    $.post('/content/chatroom/type/update', param, function (response) {
        log(response);
        if (response.success) {
            $('#table_main').bootstrapTable('refresh');
